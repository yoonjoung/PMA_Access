...
plot_ly(dtafig, x = ~xsurvey, y = ~noSDPlow, type = "bar", 
    ...
    add_segments(x = as.character("BFR1"), xend = as.character("UGR6"), 
             ...
             showlegend=FALSE)%>%
    layout(
            title = c("Percent of EAs without any linked SDPs, exluding hospitals"),
            xaxis = list(titel = ""),  
            yaxis = list(title = "(%)"),
            showlegend = FALSE
            )
...
dtafig%>%
    ...
    subplot(margin=0.05, shareX = TRUE, shareY = TRUE)%>%
    layout(
        title = c("EA-SDP link by residential area: any SDPs, exluding hospitals"),
        xaxis = list(title = "", tickfont = list(size=10)),   
        yaxis = list(title = "Percent of EAs without any linked SDPs, exluding hospitals"), 
        showlegend=FALSE
        )
