...
dtafig<-dtaSDP%>%filter(latest==TRUE)%>%filter(group=="All")
...
############### IR: SDPlow
panel <- . %>% 
    plot_ly(x=~year, y=~SDPlow_essential5_curav, type = 'bar', 
                name = "5 specific methods* available currently/3-month",
                marker = list(color = '#6baed6'),
...
figIR<-subplot(fig1, fig2, nrows=1, titleY = TRUE, margin = 0.05) %>%
    layout( 
            showlegend = FALSE, 
            yaxis = list(range = c(0, 110), tickfont = list(size=8)) )

subplot(figSDP, figIR, nrows=1, titleY = TRUE, margin = 0.05) %>%
    layout( 
            showlegend = FALSE, 
            yaxis = list(range = c(0, 110), tickfont = list(size=8)) )
```
...
figIR<-subplot(fig1, fig2, nrows=1, titleY = TRUE, margin = 0.05) %>%
    layout( 
            showlegend = FALSE, 
            yaxis = list(range = c(0, 100), tickfont = list(size=8)) )
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPall_essential5_noso)==FALSE)%>%filter(country=="Burkina Faso")
...
panel <- . %>%
    plot_ly(x=~round, y=~SDPlow_essential5_curav, type = 'scatter', mode = 'lines',
            name = "5 specific methods* available currently", 
            marker = list(color = '#6baed6'),
            line = list(color = '#6baed6')) %>%
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPlow_essential5_noso)==FALSE)%>%filter(country=="Burkina Faso")
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPall_essential5_noso)==FALSE)%>%filter(country=="Ethiopia")
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPlow_essential5_noso)==FALSE)%>%filter(country=="Ethiopia")
...
#####__Uganda__
```{r plottrendSDPtypeUG, results='asis', fig.align="left", out.width="900px", out.height="200px"}
############### SDP
panel <- . %>%
    plot_ly(x=~round, y=~essential5_curav, type = 'scatter', mode = 'lines',
            name = "5 specific methods* available currently", 
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPall_essential5_noso)==FALSE)%>%filter(country=="Uganda")
...
dtafig<-dtaIR%>%
    filter(group=="All")%>%
    filter(is.na(SDPlow_essential5_noso)==FALSE)%>%filter(country=="Uganda")
